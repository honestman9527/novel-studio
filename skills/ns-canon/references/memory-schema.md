...
当前文件夹就是小说根目录。NS 把内容分成三层：agent 项目约束、小说记忆、创作产物。具体文件职责见 [file-roles.md](file-roles.md)，创建模板见 [content-templates.md](../assets/templates/content-templates.md)。
...
| 层 | 位置 | 作用 | 不放什么 |
| --- | --- | --- | --- |
| agent 项目约束 | `AGENTS.md` / `CLAUDE.md` | agent 工作流、协作偏好、写入边界、工具和验证要求 | 人物档案、剧情大纲、资料摘录、章节进度 |
| 小说记忆 | `novel-studio/*.yaml`、`notes/*.md`、`records/*.md` | 已定事实、计划、连续性、索引、来源、长说明、进度和过程记录 | 可发布正文、对外宣传成稿、完整视觉提示词 |
...
| 进度/检查点/过程记录 | `novel-studio/records/*.md` | YAML 只留当前状态、摘要和 records 路径 |
| 对外简介和 pitch | `brief.md` | `finish.yaml` 只记状态和路径；内部长梗概进 notes |
| 调研结论和来源 | `research.yaml`、`novel-studio/records/*.md` | YAML 存结论和来源边界；records 存过程 |
...
- AGENTS/CLAUDE 是 agent 执行约束来源；小说记忆是故事事实和进度来源；创作产物是正文和展示稿来源。
- 同一事实只完整写在一个地方，其它文件只保留摘要、状态或路径。
- AGENTS/CLAUDE 不复制设定、计划、进度和资料；这些内容写入 `novel-studio/`。
- YAML 不复制 notes 全文；只保留短摘要和指向 notes 的路径。
- records 保存进度、检查点、调研/修订/迁移过程；YAML 只保留当前状态和路径。
...
- 修改 notes 长文或 records：只把摘要和路径同步回 YAML。
- 修改 AGENTS/CLAUDE：走 `$ns-guidance`，不要顺手改故事记忆。

## 文件体量

- YAML 和 frontmatter 只写短事实、状态和路径；AGENTS/CLAUDE 只写 agent 约束。
- 单个 notes 文件只处理一个主题；过长或混入多个主题时拆分。
- records 按日期或任务拆分；不要代替 YAML 记录当前 canon 状态。

---
name: ns
description: "Novel Studio 总入口。用于单本小说项目的路由和阶段协调：初始化、脑暴、框架、agent 项目约束、canon/notes/records、调研、写作、修订、文案、视觉。当用户不知道用哪个 skill、请求混合动作、需要拆分过长文件，或要从模糊需求推进到下一步时使用。"
---
...
- agent 约束文件可读；写入 AGENTS/CLAUDE 走 `$ns-guidance`。
- YAML/frontmatter 写短事实；notes 写长记忆；records 写进度和过程记录。
...
| agent 项目约束、协作边界、写入禁区 | `$ns-guidance` |
| 已定事实、进度、连续性、索引、notes、records | `$ns-canon` |
...
- “写之前要确定什么/满足写作条件吗/这个想法能开写吗”：`$ns-ideate`；方向已定但缺框架时接 `$ns-build`。
- “改一下/润色/重写/结构不行”：`$ns-revise`，由它判断轻改或结构改。
- “记住/更新设定/整理进度/写记录”：`$ns-canon`。
- “以后 agent 都要/不要这样做/这个文件别碰/写进 AGENTS 或 CLAUDE”：`$ns-guidance`。
...
- 当前文件夹是小说根目录；`novel-studio/` 是唯一小说记忆目录。

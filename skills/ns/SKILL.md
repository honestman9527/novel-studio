---
name: ns
description: "Novel Studio 小说插件总入口。用于给单本小说项目做路由和阶段协调：初始化项目、脑暴点子、搭建故事框架、维护 canon 记忆和 notes/*.md、调研素材、写新正文、修订旧文本、生成简介文案和视觉提示词。当用户不知道用哪个 skill、请求混合多个动作、需要拆分过长文件、或需要从模糊需求推进到具体下一步时使用。"
---
...
- 总入口只判断路由和阶段顺序，不直接改小说业务文件。
- `NOVEL.md` 可以读取；创建或修改必须有用户本轮明确同意。用户只说“继续写”“整理记忆”“更新设定”时，不要顺手写 `NOVEL.md`。
- YAML、frontmatter 和 `NOVEL.md` 保持短小；长人物档案、世界说明、时间线、梗概、资料摘要和风格样例写入 `novel-studio/notes/*.md`。
- 任何记忆更新会让单个文件过长时，转 `$ns-canon` 做摘要、拆分或迁移。章节正文按创作需要写，不把正文长度规则套到章节文件。
- 改正文 Markdown 后同步 frontmatter、`index.yaml` 和相关 canon；改结构化 YAML 后只同步必要的人读 Markdown，不复制长文本。
...
- “以后都要/不要再写/这个不能改/保持这种味道”：`$ns-canon`。
...
- 有根目录 `NOVEL.md` 时，所有子 skill 先读它，再生成或修改内容；写入 `NOVEL.md` 需要用户同意。
